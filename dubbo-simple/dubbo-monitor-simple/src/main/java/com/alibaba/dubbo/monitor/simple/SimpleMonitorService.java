...
public class SimpleMonitorService implements MonitorService {
			...
			for (File serviceDir : serviceDirs) {
				...
				for (File methodDir : methodDirs) {
					...
					if (elapsedChanged) {
						...
						elapsedSummary[3] = -1;
						createChart("ms/t", serviceDir.getName(), methodDir.getName(), dateDir.getName(),
								new String[] { CONSUMER, PROVIDER }, elapsedData, elapsedSummary,
								elapsedFile.getAbsolutePath());
					}
					if (successChanged) {
						...
						createChart("t/s", serviceDir.getName(), methodDir.getName(), dateDir.getName(),
								new String[] { CONSUMER, PROVIDER }, successData, successSummary,
	...
	private static void createChart(String key, String service, String method, String date, String[] types,
		...
		JFreeChart jfreechart = ChartFactory.createTimeSeriesChart("max: " + numberFormat.format(summary[0])
				+ (summary[1] >= 0 ? " min: " + numberFormat.format(summary[1]) : "") + " avg: "
				+ numberFormat.format(summary[2]) + (summary[3] >= 0 ? " sum: " + numberFormat.format(summary[3]) : ""),
